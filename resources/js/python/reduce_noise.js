...
function apply_event() {
    ...
    r.on('fileProgress', function (file) {
        ...
        document.getElementById("reduce_noise_btn").disabled=true;
    });
    r.on('fileSuccess', function (file, response) {
        ...
        document.getElementById("reduce_noise_btn").disabled=false;
        if (run_python) {
            document.getElementById("upload_file_btn").disabled = true;
            document.getElementById("reduce_noise_btn").addEventListener("click",()=>{
...
function run_python_file() {
    $.ajax({
        ...
        url:'/python/reduce_noise/run_python_file',
        data: {
        ...
        success: function(data) {
            ...
            for (let i = 0; i < all_file.length; i++) {
                ...
                function download_file() {
                    window.open(("reduce_noise/file/"+data[all_file[i]].url), "_blank");
